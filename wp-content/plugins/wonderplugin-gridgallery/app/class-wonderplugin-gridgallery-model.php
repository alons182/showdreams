...
class WonderPlugin_Gridgallery_Model {
	...
	function generate_body_code($id, $has_wrapper) {
		...
		if ($item_row != null)
		{
			...
			$boolOptions = array('random', 'shownavigation', 'hoverzoomin', 'hoverzoominimageonly', 'hoverzoominimagecenter', 'hoverfade', 'responsive', 'mediumscreen', 'smallscreen', 'showtitle', 'showtexttitle', 'showtextdescription', 'showtextbutton', 'lightboxnogroup', 'lightboxshowtitle', 'lightboxshowdescription', 'lightboxresponsive', 'circularimage', 'firstimage', 'textinsidespace');
			...
			$valOptions = array('thumbwidth', 'thumbheight', 'thumbtopmargin', 'thumbbottommargin', 'barheight', 'titlebottomcss', 'descriptionbottomcss', 'googleanalyticsaccount', 'gap', 'margin', 'borderradius', 
					...
					'videoplaybutton', 'column', 'mediumcolumn', 'mediumscreensize', 'smallcolumn', 'smallscreensize', 'titlemode', 'titleeffect', 'titleeffectduration', 'titleheight'
					);
			...
			if (isset($data->slides) && count($data->slides) > 0)
			{
				...
				if (isset($templates) && count($templates) > 0 && count($templates[0]) > 0)
				{
					...
					{							
						...
						{
							$code_template .= '<a class="wpgridlightbox wpgridlightbox-' . $id . '" data-thumbnail="' . $slide->thumbnail . '"';
						...
						else
						{
							...
							{
								$code_template .= '" class="wpgridlightbox wpgridlightbox-' . $id . '"';
						...
						if (isset($slide->button) && strlen($slide->button) > 0)
						{
							...
							$code_template .= ' data-button="' . str_replace("\"", "&quot;", $button_code) . '"';
						}
						
						$code_template .= '>';
							
						$code_template .= '<img class="wonderplugin-gridgallery-item-img" src="';
